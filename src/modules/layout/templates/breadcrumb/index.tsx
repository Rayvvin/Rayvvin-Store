    ...
    <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse overflow-x-auto whitespace-nowrap">
      ...
      {pathnames.length >= 2 && (
        <li className="inline-flex items-center">
          ...
          <button className="inline-flex items-center text-sm font-normal text-gray-700 hover:text-[#3D8B7A] capitalize" onClick={() => setNavDropdownToggle(!navDropdownToggle)}>
            {/* {pathnames[0].length > 10 ? `${pathnames[0].substring(0, 10)}...` : pathnames[0]}{" "} */}
            ...
          </button>
          <div
            className={clsx(
              "absolute top-[7.3rem] sm:top-[12rem] z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44",
              {
                hidden: !navDropdownToggle,
              }
            )}
          >
      ...
      {pathnames.length >= 1 && (
        <li className="inline-flex items-center">
          ...
          >
            {pathnames[pathnames.length - 1].length > 10
              ? `${pathnames[pathnames.length - 1].substring(0, 10)}...`
