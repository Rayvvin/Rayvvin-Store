...
const FeaturedMarkets = (props) => {
  ...
  const {
    ...
    loading,
    error,
  } = useStateMarkets(state?.stateName)
  ...
  useEffect(() => {
    // console.log(state)
    if (state) {
      setRegions(state?.markets.map(mrkt => {return {...mrkt, state_handle: state?.handle}}))
    }
  }, [state])
      ...
      all_markets = all_markets.concat(
        state.markets.map((mrkt) => {
          return {
            ...
            state: state[Object.keys(state)[0]],
            state_id: state[Object.keys(state)[0]]
              .split(" ")[0]
              .toLowerCase()
              .replace(/\s/g, "-"),
          }
